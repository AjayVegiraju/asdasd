...
public:
  ...
  static bool add(const string &label);

  // @return true if vertex is in the graph
  static bool contains(const string &label);
  ...
  bool connect(const string &from, const string &to, int weight = 0) const;
  ...
  int vertexDegree(const string &label);
  ...
  pair<map<string, int>, map<string, string>> dijkstra(const string &startLabel);
