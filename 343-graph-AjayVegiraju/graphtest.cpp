...
void testGraphBasic() {
  ...
  cout << "testGraphBasic DONE!" << endl;
}
//---------------------------------------------------------------------------------
void testGraph0DFS() {
  cout << "testGraph0DFS" << endl;
  Graph d;
  if (!d.readFile("graph0.txt")) {
    return;
  }
  cout << "Readfile works" << endl;
  assert(d.contains("A") && "A in graph");
  assert(d.contains("B") && "B in graph");
  assert(d.contains("C") && "C in graph");
  assert(d.getEdgesAsString("A") == "B(1),C(8)");
  assert(d.getEdgesAsString("B") == "C(3)");
  assert(d.getEdgesAsString("C").empty());

  d.dfs("A", vertexPrinter);
  assert(globalSS.str() == "ABC" && "starting from A");

  globalSS.str("");
  d.dfs("B", vertexPrinter);
  assert(globalSS.str() == "BC" && "starting from B");

  globalSS.str("");
  d.dfs("C", vertexPrinter);
  assert(globalSS.str() == "C" && "starting from C");

  globalSS.str("");
  d.dfs("X", vertexPrinter);
  assert(globalSS.str().empty() && "starting from X");

  cout << "testGraph0DFS done" << endl;
}
//---------------------------------------------------------------------------------
void testGraph0BFS() {
